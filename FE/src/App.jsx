...
export function App() {
  ...
  const [productCategory, setProductCategory] = useState("Tat ca");
  ...
  function addToCart(product, quantity = 1) {
    ...
    setCart((current) => {
      const next = {
        ...current,
        [product.id]: Math.min(product.stock, (current[product.id] || 0) + Number(quantity || 1))
      };
  ...
  const detailProduct = useMemo(() => {
    ...
    return products.find((item) => item.id === selectedProduct.id) || selectedProduct;
