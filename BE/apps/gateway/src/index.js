...
app.post("/api/ai/product-suggestions", aiRateLimiter, async (req, res, next) => {
  try {
    ...
    const suggestions = filterSuggestedProducts(products, normalizedCriteria).slice(0, 12);
...
function normalizeCriteria(criteria, description) {
  ...
  const keywords = normalizeArray(criteria?.keywords).length ? normalizeArray(criteria.keywords) : fallback.keywords;
  const origins = normalizeArray(criteria?.origins);
  const categories = normalizeArray(criteria?.categories);
  let minPrice = toNullableNumber(criteria?.minPrice ?? fallback.minPrice);
  let maxPrice = toNullableNumber(criteria?.maxPrice ?? fallback.maxPrice);
...
function filterSuggestedProducts(products, criteria) {
  return products
    .filter((product) => {
      ...
      const keywordMatch =
        criteria.keywords.length === 0 || criteria.keywords.some((keyword) => text.includes(normalizeText(keyword)));
      const originMatch =
        criteria.origins.length === 0 ||
        criteria.origins.some((origin) => normalizeText(product.origin || "").includes(normalizeText(origin)));
      const categoryMatch =
        criteria.categories.length === 0 ||
        criteria.categories.some((category) => normalizeText(product.category || "").includes(normalizeText(category)));
